...
type InferOutput<TSchema extends ZodTypeAny | undefined> =
  TSchema extends ZodArray<infer TItem>
    ? z.infer<TItem>[]
  ...
  build(): DBService<P, R> {
    ...
    const service: DBServiceFn<P, R> = async (...args: [DBContext] | [DBContext, P]): Promise<R> => {
      ...
      if (self._schema && self._schema instanceof z.ZodArray) {
        ...
        const transformed = data.map((row) => transform(row as Record<string, unknown>));
 ...
 */
export const createDBService = <P = void>(): DBService<P, unknown> =>
  new DBServiceBuilder<P, undefined>().build();
